...
let patch = async (req, res) => {
  try {
    let data2 = [req.body, req.params.shipping_id]
    query2 = 'UPDATE  cart SET ? WHERE shipping_id = ? '
...
let del = async (req, res) => {
  try {
    let data = req.params.shipping_id
    let query2 = "DELETE from cart WHERE shipping_id = ?"
